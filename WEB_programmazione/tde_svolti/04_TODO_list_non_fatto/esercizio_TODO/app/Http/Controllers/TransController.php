...
{  /**
    ...
    public function toggleTipo(Request $request) {
    $transId = $request->input('id');
    $dl = new DataLayer();
    $trans = $dl->findTransById($transId);

    if (!$trans) {
        return response()->json(['error' => 'Transazione non trovata'], 404);
    }

    $trans->tipo = $trans->tipo === 'entrata' ? 'spesa' : 'entrata';
    $trans->save();
    ...
    public function destroyAjax(Request $request) {
        $dl = new DataLayer();
        //Log::info('Chiamata AJAX ricevuta nel destroy', ['dati' => $request->all()]);
        Log::info('Chiamata AJAX ricevuta nel destroy');
        $dl->deleteTrans($request->input('id'));
