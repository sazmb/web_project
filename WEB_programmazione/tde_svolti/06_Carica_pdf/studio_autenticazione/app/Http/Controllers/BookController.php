...
class BookController extends Controller
{
    ...
    {
        Log::info('Displaying the list of books');
        $dl = new DataLayer();
        // $booksList = $dl->listBooks();
        $booksList = $dl->listBooks(auth()->user()->id);
    ...
    public function create()
    {
        ...
        $dl = new DataLayer();
        $authorList = $dl->listAuthors(auth()->user()->id);
    ...
    public function store(Request $request)
    {
        ...
        $request->input('author_id'),$selectedCategories, auth()->user()->id);
    ...
    {
        Log::info('Displaying the details of a book');
        $dl = new DataLayer();
        $book = $dl->findBookById($id, auth()->user()->id);
    ...
    {
        Log::info('Displaying the form for editing a book');
        $dl = new DataLayer();
        $authorList = $dl->listAuthors(auth()->user()->id);
        $book = $dl->findBookById($id, auth()->user()->id);
        $categories = $dl->getAllCategories();
    ...
    {
        Log::info('Displaying the confirmation for deleting a book');
        $dl = new DataLayer();
        $book = $dl->findBookById($id, auth()->user()->id);
        if ($book !== null) {
