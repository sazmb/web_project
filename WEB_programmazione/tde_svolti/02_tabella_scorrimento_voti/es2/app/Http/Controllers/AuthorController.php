...
class AuthorController extends Controller
{
    ...
    {
        $dl = new DataLayer();
        $authorsList = $dl->listAuthors(auth()->user()->id);
    ...
    public function store(Request $request)
    {
        ...
        //abort(501);
        $dl = new DataLayer();
        $dl->addAuthor($request->input('firstName'),$request->input('lastName'),auth()->user()->id);
    ...
    {
        $dl = new DataLayer();
        $author = $dl->findAuthorById($id, auth()->user()->id);
