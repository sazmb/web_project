        ...
        <nav class="navbar navbar-expand-lg bg-body-tertiary">
            <div class="container-fluid">
              ...
              <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle @yield('active_MyLibrary')" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            @lang('messages.my_library')
                        </a>
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="{{ route('book.index') }}">{{ trans('messages.books_list') }}</a></li>
                            <li><a class="dropdown-item" href="{{ route('author.index') }}">{{ trans('messages.authors_list') }}</a></li>
                        </ul>
                    </li>
                    @endif
                ...
                <ul class="navbar-nav">
                    @if(auth()->check())
                    <li class="nav-item"><i>{{ trans('messages.welcome') }} {{ auth()->user()->name }}</i>
                    ...
                    @else
                    <li class="nav-item"><a href="{{ route('login') }}"><i class="bi bi-person-lock"></i></a></li>
                    @endif
                </ul>
              </div>
            </div>
